...
vector<SeedPair_t> GenerateSimplePairsFromCommonKmers(int MinSeedLength, vector<KmerPair_t>& KmerPairVec)
{
	SeedPair_t SeedPair;
	...
	for (i = 0; i < num;)
	{
		...
		if ((l = KmerSize + (j - 1 - i)) >= MinSeedLength)
		{
			//printf("Seed found! r=%d, l=%d\n\n", KmerPairVec[i].rPos, l);
			SeedPair.rPos = KmerPairVec[i].rPos;
			SeedPair.gPos = KmerPairVec[i].gPos;
			SeedPair.PosDiff = KmerPairVec[i].PosDiff;
			SeedPair.rLen = SeedPair.gLen = l;
			SimplePairVec.push_back(SeedPair);
		}
		i = j;
	}
